...
\item [Covariance and Independence] - If two random variables are independent, then they are uncorrelated. The inverse is not necessarily true, except in the case of Multivariate Normal, where uncorrelated \emph{does} imply independence.
...
\subsubsection{Negative hypergeometric} PMF is $\frac{1}{g+1}$ where g is the event you want
...
\begin{center}
    ...
    ~ & \textbf{CDF} & \textbf{MGF}  \\
    \midrule
        \textbf{Binomial} &  approx w/ normal for large n & $(q + pe^t)^n$ \\
        \textbf{Geom} & $1-q^{s}$ & $\frac{p}{1-qe^t}, qe^t < 1$ \\
        \textbf{Unif} & $\frac{x-a}{b-a}$ & $\frac{e^{tb}-e^{ta}}{t(b-a)}$ \\
        \textbf{Expo} & $1 - e^{-\lambda x}$ & $\frac{\lambda}{\lambda - t}, t < \lambda$ \\
    \bottomrule
    \end{tabular}
\end{center}
